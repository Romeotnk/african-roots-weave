...
export const listProducts = asyncHandler(async (req, res) => {
  ...
  const where: Prisma.ProductWhereInput = {
    ...
    category: Object.values(MedCategory).includes(req.query.category as MedCategory)
      ...
      : undefined,
    type: Object.values(ProductType).includes(req.query.type as ProductType) ? (req.query.type as ProductType) : undefined,
    ...
    seller:
      typeof req.query.location === 'string'
        ? { professionalProfile: { location: { contains: req.query.location, mode: 'insensitive' } } }
        : undefined,
  };
...
export const updateProduct = asyncHandler(async (req, res) => {
  ...
  if (existing.sellerId !== req.user.id && req.user.role !== Role.ADMIN) throw new ApiError(403, 'Forbidden');
...
export const uploadProductImages = asyncHandler(async (req, res) => {
  ...
  const product = await prisma.product.findUnique({ where: { id: req.params.id }, select: { sellerId: true, images: true } });
  if (!product) throw new ApiError(404, 'Product not found');
  if (product.sellerId !== req.user.id && req.user.role !== Role.ADMIN) throw new ApiError(403, 'Forbidden');
