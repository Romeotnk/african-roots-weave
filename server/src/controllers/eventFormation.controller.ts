...
export const registerEvent = asyncHandler(async (req, res) => {
  if (!req.user) throw new ApiError(401, 'Authentication required');
  const registration = await prisma.eventRegistration.create({ data: { eventId: req.params.id, userId: req.user.id } });
...
export const unregisterEvent = asyncHandler(async (req, res) => {
  if (!req.user) throw new ApiError(401, 'Authentication required');
  await prisma.eventRegistration.delete({ where: { eventId_userId: { eventId: req.params.id, userId: req.user.id } } });
...
export const listFormations = asyncHandler(async (req, res) => {
  ...
  res.json(apiResponse(true, formations, 'Formations retrieved', paginationMeta(page, limit, total)));
});
...
export const createFormation = asyncHandler(async (req, res) => {
  if (!req.user) throw new ApiError(401, 'Authentication required');
  const formation = await prisma.formation.create({ data: { ...req.body, createdById: req.user.id } });
...
export const downloadFormation = asyncHandler(async (req, res) => {
  ...
  res.json(apiResponse(true, { ...formation, signedUrl: formation.fileUrl }, 'Formation download ready'));
});
