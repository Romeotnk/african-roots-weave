...
export const cartQuantityValidator = [param('itemId').isString().notEmpty(), body('quantity').isInt({ min: 1 })];
...
export const reasonValidator = [param('id').isString().notEmpty(), body('reason').isString().isLength({ min: 5 }).trim()];
...
export const couponValidateValidator = [body('code').isString().isLength({ min: 3, max: 40 }).trim().escape()];
