...
async function main() {
  ...
  const specialties = [
    ...
    "Savoirs peuls",
    "Herboristerie sahelienne",
    "Rites et soins communautaires",
  ];
  for (let index = 1; index <= 5; index += 1) {
    const professional = await prisma.user.create({
      data: {
        ...
        mlmNode: {
          create: {
            affiliateCode: `PRO${index}IWO`,
            level: 2,
            parent: { connect: { userId: users[index - 1].id } },
          },
        },
        professionalProfile: {
          create: {
            ...
            initiationPath:
              "Apprentissage aupres des anciens, terrain rural et consultations accompagnees.",
            ...
            philosophy:
              "Respect du vivant, prudence therapeutique et dialogue avec la medecine moderne.",
        ...
        subscription: {
          create: {
            plan: index > 3 ? "PRO" : "BASIC",
            price: index > 3 ? 25000 : 10000,
            startDate: new Date(),
            maxListings: 25,
            maxDownloads: 50,
          },
  ...
  for (let index = 0; index < 20; index += 1) {
    await prisma.product.create({
      data: {
        ...
        description:
          "Produit documente par un professionnel verifie, avec conseils d usage et precautions.",
    ...
    await prisma.plantMonograph.create({
      data: {
        ...
        botanicalDescription:
          "Description botanique synthetique avec habitat, feuilles, fleurs et usages connus.",
        activeCompounds: "Polyphenols, flavonoides, mineraux et composes bioactifs documentes.",
        therapeuticIndications:
          "Usages traditionnels de soutien, a encadrer par un professionnel qualifie.",
        dosage: "Dosage indicatif a adapter selon le contexte, l age et les contre-indications.",
        contraindications:
          "Grossesse, allaitement, allergies connues ou interactions medicamenteuses possibles.",
  ...
  for (let index = 1; index <= 10; index += 1) {
    await prisma.article.create({
      data: {
        ...
        content:
          "Contenu editorial long sur les savoirs endogenes, la prevention et les pratiques culturelles.",
  ...
  for (let index = 1; index <= 5; index += 1) {
    const question = await prisma.question.create({
      data: {
        ...
        title: `Question sante communautaire ${index}`,
        content: "Comment mieux documenter cette pratique ?",
        category: "General",
        tags: ["discussion"],
        attachments: [],
      },
    ...
    await prisma.answer.create({
      data: {
        ...
        authorId: professionals[index % professionals.length].id,
        content: "Reponse prudente avec orientation vers un professionnel verifie.",
        attachments: [],
        voteCount: 3,
      },
    });
    await prisma.vote.create({
      data: { userId: users[0].id, targetId: question.id, targetType: "QUESTION", value: 1 },
    });
  }
  ...
  await prisma.event.createMany({
    data: [
      {
        title: "Webinar pharmacopée",
        description: "Session en ligne.",
      ...
      {
        title: "Salon Iwosan",
        description: "Rencontre panafricaine.",
  ...
  await prisma.siteConfig.createMany({
    data: [
      ...
      { key: "commission.globalRate", value: "0.10", updatedById: admin.id },
      {
        ...
        value: JSON.stringify({ maxListings: 5, maxDownloads: 10 }),
        updatedById: admin.id,
      },
      {
        key: "subscription.BASIC",
        value: JSON.stringify({ price: 10000, maxListings: 25, maxDownloads: 50 }),
        updatedById: admin.id,
      },
      {
        key: "subscription.PRO",
        value: JSON.stringify({ price: 25000, maxListings: 100, maxDownloads: 200 }),
        updatedById: admin.id,
      },
      {
        key: "subscription.EXPERT",
        value: JSON.stringify({ price: 50000, maxListings: 999, maxDownloads: 999 }),
        updatedById: admin.id,
      },
    ],
  });
