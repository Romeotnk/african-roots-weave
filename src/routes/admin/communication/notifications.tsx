import { createFileRoute } from "@tanstack/react-router";
import { useState } from "react";
import { useTranslation } from "react-i18next";
import { AdminCard, AdminLayout } from "@/components/admin/AdminLayout";
import { useBroadcastAdminNotification } from "@/hooks/useAdminApi";

export const Route = createFileRoute("/admin/communication/notifications")({
  head: () => ({ meta: [{ title: "Admin notifications - IWOSAN" }] }),
  component: AdminNotifications,
});

function AdminNotifications() {
  const { t } = useTranslation();
  const audiences = [
    { value: "all", label: t("admin.pushNotifications.audienceAll") },
    { value: "users", label: t("admin.pushNotifications.audienceUsers") },
    { value: "professionals", label: t("admin.pushNotifications.audienceProfessionals") },
  ];
  const broadcast = useBroadcastAdminNotification();
  const [audience, setAudience] = useState("all");
  const [title, setTitle] = useState("");
  const [message, setMessage] = useState("");
  const [link, setLink] = useState("");
  const [notice, setNotice] = useState("");

  const canSend = title.trim().length > 0 && message.trim().length > 0 && !broadcast.isPending;

  return (
    <AdminLayout title={t("admin.pushNotifications.title")} description={t("admin.pushNotifications.description")}>
      {notice && <div className="mb-4 rounded-lg bg-emerald-500/15 p-3 text-[13px] text-emerald-200">{notice}</div>}
      {broadcast.isError && (
        <div className="mb-4 rounded-lg bg-red-500/15 p-3 text-[13px] text-red-200">
          {broadcast.error instanceof Error ? broadcast.error.message : t("admin.pushNotifications.sendError")}
        </div>
      )}

      <AdminCard>
        <h2 className="mb-4 text-[18px] font-bold text-white">{t("admin.pushNotifications.sendNotification")}</h2>
        <div className="grid gap-4 md:grid-cols-2">
          <label className="text-[13px] text-slate-300">
            {t("admin.pushNotifications.audience")}
            <select
              value={audience}
              onChange={(event) => setAudience(event.target.value)}
              className="mt-1 h-10 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-[13px] text-white outline-none"
            >
              {audiences.map((item) => (
                <option key={item.value} value={item.value} className="bg-[#151529]">
                  {item.label}
                </option>
              ))}
            </select>
          </label>
          <label className="text-[13px] text-slate-300">
            {t("admin.pushNotifications.linkOptional")}
            <input
              value={link}
              onChange={(event) => setLink(event.target.value)}
              placeholder={t("admin.pushNotifications.linkPlaceholder")}
              className="mt-1 h-10 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-[13px] text-white outline-none"
            />
          </label>
          <label className="text-[13px] text-slate-300 md:col-span-2">
            {t("admin.pushNotifications.titleLabel")}
            <input
              value={title}
              onChange={(event) => setTitle(event.target.value)}
              placeholder={t("admin.pushNotifications.titlePlaceholder")}
              className="mt-1 h-10 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-[13px] text-white outline-none"
            />
          </label>
          <label className="text-[13px] text-slate-300 md:col-span-2">
            {t("admin.pushNotifications.messageLabel")}
            <textarea
              value={message}
              onChange={(event) => setMessage(event.target.value)}
              placeholder={t("admin.pushNotifications.messagePlaceholder")}
              className="mt-1 min-h-32 w-full rounded-lg border border-white/10 bg-white/5 p-3 text-[13px] text-white outline-none"
            />
          </label>
        </div>
        <button
          type="button"
          disabled={!canSend}
          onClick={() =>
            broadcast.mutate(
              { audience, title: title.trim(), message: message.trim(), link: link.trim() || undefined },
              {
                onSuccess: (result) => {
                  setNotice(t("admin.pushNotifications.sentTo", { count: result.data?.recipientCount ?? 0 }));
                  setTitle("");
                  setMessage("");
                  setLink("");
                },
              },
            )
          }
          className="mt-4 rounded-full bg-emerald-400 px-4 py-2 text-[12px] font-bold text-[#111827] disabled:opacity-50"
        >
          {broadcast.isPending ? t("admin.pushNotifications.sending") : t("admin.pushNotifications.send")}
        </button>
      </AdminCard>
    </AdminLayout>
  );
}
