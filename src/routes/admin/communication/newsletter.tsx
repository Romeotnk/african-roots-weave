...
function AdminNewsletterPage() {
  ...
  const send = () => {
    if (!subject.trim() || !body.trim()) {
      setNotice("Renseignez un objet et un message avant l'envoi.");
      return;
    }
    const html = body
      .trim()
    ...
    sendNewsletter.mutate(
      { subject: subject.trim(), html },
      {
        onSuccess: (response) => {
          ...
          setNotice(`Newsletter envoyée à ${sentCount} abonné(s).`);
          setSubject("");
          setBody("");
        },
        onError: (error) => setNotice(error instanceof Error ? error.message : "Échec de l'envoi."),
      },
    );
  };

  return (
    <AdminLayout title="Newsletter" description="Abonnés et envoi de campagnes.">
      <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
        <AdminCard className="p-0">
          <div className="max-h-[520px] overflow-x-auto overflow-y-auto">
            <table className="w-full min-w-[420px] text-left text-[13px]">
              <thead className="bg-white/10 text-slate-300">
                <tr>{["Email", "Inscription", "Statut"].map((header) => <th key={header} className="px-4 py-3 font-bold">{header}</th>)}</tr>
              </thead>
              <tbody>
                {subscribersQuery.isLoading && <tr><td colSpan={3} className="px-4 py-6 text-center text-slate-400">Chargement...</td></tr>}
                {!subscribersQuery.isLoading && subscribers.length === 0 && (
                  <tr><td colSpan={3} className="px-4 py-6 text-center text-slate-400">Aucun abonné pour le moment.</td></tr>
                )}
                  ...
                  <tr key={subscriber.id} className="border-t border-white/10">
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-1 text-[11px] font-bold ${subscriber.isActive ? "bg-emerald-500/20 text-emerald-300" : "bg-white/10 text-slate-400"}`}>
                        {subscriber.isActive ? "Actif" : "Désinscrit"}
                      </span>
                    </td>
                  </tr>
                ))}
        ...
        <AdminCard>
          <h2 className="mb-4 text-[18px] font-bold text-white">Campagne</h2>
          <p className="mb-3 text-[12px] text-slate-500">{activeCount} abonné(s) actif(s) recevront cet envoi.</p>
          <label className="block">
            <span className="mb-1 block text-[12px] font-bold uppercase tracking-[0.1em] text-slate-400">Objet</span>
          ...
          <label className="mt-3 block">
            <span className="mb-1 block text-[12px] font-bold uppercase tracking-[0.1em] text-slate-400">Message</span>
          ...
          >
            {sendNewsletter.isPending ? "Envoi..." : "Envoyer"}
          </button>
        </AdminCard>
      </div>
    </AdminLayout>
