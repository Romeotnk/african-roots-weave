...
function AdminLogsPage() {
  ...
  return (
    <AdminLayout title="Logs d'activité" description="Journal admin en lecture seule.">
      {auditQuery.isLoading && <p className="text-[13px] text-slate-400">Chargement...</p>}
      {auditQuery.isError && (
        <p className="text-[13px] text-red-300">
          Impossible de charger le journal — cette page est réservée au rôle Super Admin.
        </p>
      )}
        ...
        <div className="overflow-x-auto rounded-[12px] border border-white/10">
          <table className="w-full min-w-[820px] text-left text-[13px]">
            <thead className="bg-white/10 text-slate-300">
              <tr>{["Date", "Admin", "Rôle", "Action", "Cible", "IP"].map((header) => <th key={header} className="px-4 py-3 font-bold">{header}</th>)}</tr>
            </thead>
            <tbody>
              {logs.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-6 text-center text-slate-400">Aucune action enregistrée.</td></tr>
              )}
              {logs.map((log) => (
                <tr key={log.id} className="border-t border-white/10">
                  <td className="px-4 py-3 text-slate-200">{new Date(log.createdAt).toLocaleString("fr-FR")}</td>
                  <td className="px-4 py-3 text-slate-200">{log.user?.email ?? "Système"}</td>
