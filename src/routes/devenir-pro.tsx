...
const slots = ["Matin", "AprÃ¨s-midi", "Soir"];

function BecomePro() {
  const [country, setCountry] = useState("BJ");
  const [online, setOnline] = useState(true);
  const [selectedSlots, setSelectedSlots] = useState<string[]>(["Lundi-Matin", "Mercredi-AprÃ¨s-midi"]);
  ...
  const submitProfile = () => {
    ...
    setSubmitted(true);
    setFormMessage("Profil soumis en moderation mock. L'equipe pourra verifier les documents avant publication.");
  };
    ...
    <main className="min-h-screen bg-[var(--brand-bg)]">
      <section className="border-b border-[var(--brand-border-light)] bg-white">
        <div className="container-iwosan py-8">
          ...
          <h1 className="mt-2 text-[32px] md:text-[42px]">CrÃ©er mon profil professionnel</h1>
          <p className="mt-2 max-w-2xl text-[14px] text-[var(--color-text-muted)]">
            Formulaire mock complet pour praticiens, documents et disponibilitÃ©s. Validation API plus tard.
          </p>
        </div>
      </section>
        ...
        >
          <div className="rounded-[12px] border border-[var(--brand-border-light)] bg-white p-5">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              ...
              <input placeholder="SpÃ©cialitÃ©s secondaires (max 5)" className="h-11 rounded-lg border border-[var(--brand-border)] px-4" />
              <input placeholder="Langues pratiquÃ©es" className="h-11 rounded-lg border border-[var(--brand-border)] px-4" />
            </div>
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <label className="flex min-h-[130px] cursor-pointer flex-col items-center justify-center rounded-[12px] border-2 border-dashed border-[var(--brand-border)] bg-[var(--brand-surface-alt)]">
                ...
                <span className="mt-2 text-[13px] font-semibold">Galerie 5 Ã  10 photos</span>
          ...
          <div className="rounded-[12px] border border-[var(--brand-border-light)] bg-white p-5">
            <h2 className="text-[20px] font-bold">RÃ©cit professionnel</h2>
            <div className="mt-5 space-y-3">
              <textarea required minLength={300} rows={6} placeholder="Histoire / biographie narrative, minimum 300 caractÃ¨res" className="w-full rounded-lg border border-[var(--brand-border)] px-4 py-3" />
              <textarea rows={4} placeholder="Formation et initiation : comment, oÃ¹, avec qui" className="w-full rounded-lg border border-[var(--brand-border)] px-4 py-3" />
              <div className="flex gap-2">
                <input value={treatedInput} onChange={(event) => setTreatedInput(event.target.value)} placeholder="SpÃ©cialitÃ© traitÃ©e" className="h-10 flex-1 rounded-lg border border-[var(--brand-border)] px-3" />
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <input type="number" min="0" max="100" placeholder="Taux de rÃ©ussite auto-dÃ©clarÃ© (%)" className="h-11 rounded-lg border border-[var(--brand-border)] px-4" />
                <input type="number" min="0" placeholder="AnnÃ©es d'expÃ©rience" className="h-11 rounded-lg border border-[var(--brand-border)] px-4" />
              </div>
              <textarea rows={3} placeholder="Explications obligatoires du taux annoncÃ©" className="w-full rounded-lg border border-[var(--brand-border)] px-4 py-3" />
            </div>
          </div>

          <div className="rounded-[12px] border border-[var(--brand-border-light)] bg-white p-5">
            <h2 className="flex items-center gap-2 text-[20px] font-bold"><MapPin size={20} /> Localisation et disponibilitÃ©</h2>
            <div className="mt-5 grid gap-3 md:grid-cols-2">
