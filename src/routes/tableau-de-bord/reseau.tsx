...
const toCsv = (members: FlatMember[], header: string) => {
...
function ReseauPage() {
  ...
  const copyLink = async () => {
    ...
    setError("");
    if (!affiliate?.link) {
      setError(t("dashboard.network.linkNotAvailable"));
      return;
    }
    try {
      await navigator.clipboard.writeText(affiliate.link);
      setMessage(t("dashboard.network.linkCopied"));
    } catch {
      setError(t("dashboard.network.copyError"));
    }
  };

  const exportList = () => {
    if (members.length === 0) {
      setError(t("dashboard.network.nothingToExport"));
      return;
    }
    setError("");
    const blob = new Blob([toCsv(members, t("dashboard.network.csvHeader"))], { type: "text/csv;charset=utf-8;" });
    ...
    setMessage(t("dashboard.network.exportDownloaded"));
  };
      ...
      <AccountLayout
        ...
        actions={
          <button type="button" onClick={exportList} className="btn-secondary h-11 px-5 text-[14px]">
            <Download size={17} /> {t("dashboard.network.export")}
          </button>
        }
      >
          <div className="grid gap-4 md:grid-cols-3">
            <StatCard icon={Users} label={t("dashboard.network.referrals")} value={String(members.length)} />
            <StatCard icon={Network} label={t("dashboard.network.level2Plus")} value={String(levelTwoCount)} />
            <StatCard icon={TrendingUp} label={t("dashboard.network.networkEarnings")} value={formatMoney(totalVolume)} />
          </div>

          <div className="mt-6 grid gap-4 lg:grid-cols-[360px_1fr]">
            <aside className="rounded-[8px] border border-[var(--brand-border-light)] bg-white p-5">
              <h2 className="text-[18px] font-bold">{t("dashboard.network.inviteTitle")}</h2>
              <p className="mt-2 text-[13px] leading-6 text-[var(--color-text-muted)]">
                {t("dashboard.network.inviteDesc")}
              </p>
              {affiliate?.link && (
              ...
              <button type="button" onClick={copyLink} className="btn-primary mt-4 h-11 w-full text-[14px]">
                <Copy size={17} /> {t("dashboard.network.copyMyLink")}
              </button>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <label className="relative block min-w-0 flex-1">
                  ...
                  <input
                    ...
                    placeholder={t("dashboard.network.searchPlaceholder")}
                ...
                <div className="flex flex-wrap gap-2">
                  {(["all", 1, 2, 3] as const).map((level) => (
                    ...
                    >
                      {level === "all" ? t("dashboard.network.all") : t("dashboard.network.level", { level })}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-5 space-y-3">
                {isLoading ? (
                  ...
                  <div className="rounded-[8px] border border-[var(--brand-border-light)] bg-[var(--brand-surface-alt)] p-8 text-center">
                    ...
                    <h2 className="mt-4 text-[20px] font-bold">{t("dashboard.network.emptyTitle")}</h2>
                    <p className="mt-2 text-[14px] text-[var(--color-text-muted)]">{t("dashboard.network.emptyDesc")}</p>
                  </div>
                ) : (
                      ...
                      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            <h2 className="text-[17px] font-bold text-[var(--color-text-primary)]">{member.name || t("dashboard.network.defaultMemberName")}</h2>
                            <span className="rounded-full border border-[var(--brand-border-light)] bg-[var(--brand-surface-alt)] px-3 py-1 text-[12px] font-bold text-[var(--color-text-secondary)]">
                              {t("dashboard.network.level", { level: member.level })}
                            </span>
                          </div>
                          <p className="mt-1 text-[13px] text-[var(--color-text-muted)]">
                            {t("dashboard.network.memberSummary", { country: member.country || t("dashboard.network.unknownCountry"), count: member.downlineCount, date: formatDate(member.joinedAt) })}
                          </p>
                        </div>
