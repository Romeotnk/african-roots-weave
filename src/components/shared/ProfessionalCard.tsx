  ...
  return (
    <article className="overflow-hidden rounded-[20px] border border-[var(--brand-border-light)] bg-white shadow-iwosan-sm transition hover:-translate-y-1 hover:border-[var(--brand-primary)] hover:shadow-iwosan-lg">
      <div className="relative h-[120px] overflow-hidden">
        ...
        {pro.verified && <span className="absolute right-3 top-3 rounded-full bg-[var(--brand-gold)] px-3 py-1 text-[11px] font-bold text-white">{t("professionalCard.verified")}</span>}
      </div>
      <div className="px-4 pb-5 text-center">
        ...
        <p className="mt-2 inline-flex items-center gap-1 text-[12px] text-[var(--color-text-muted)]">
          <MapPin size={12} />{pro.location}, {pro.country}
          {typeof pro.distanceKm === "number" && <span className="font-semibold text-[var(--brand-primary)]"> · {pro.distanceKm < 1 ? t("professionalCard.lessThan1Km") : t("professionalCard.kmAway", { km: Math.round(pro.distanceKm) })}</span>}
        </p>
